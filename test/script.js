document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- 1. PARSEJAR I NETEJAR GPX ROBUSTAMENT ---
    function parseGPX(gpxText) {
        const parser = new DOMParser();
        const xmlDoc = parser.parseFromString(gpxText, "text/xml");
        
        // Suportem tant <trkseg> com <trkpt> directes
        ...
        trkpts.forEach((pt, index) => {
            ...
            if (prevPoint) {
                ...
                // Si el punt està a menys de 2 metres o fa un salt anòmal de més de 3km de sobte, el filtrem per evitar corrupcions
                if (distIncrement < 0.002 || distIncrement > 3.0) {
                    return; 
                }
    ...
    // --- 2. DETECCIÓ DE PORTS AMB PROTECCIÓ CONTRA PENDENTS ABSURDS ---
    function detectCols(points) {
        // Suavitzat mitjançant finestra llarga per eliminar pics de GPS falsos
        const windowSize = 9;
        ...
        // Calculem el pendent basat en blocs de distància coherents (p.ex. comparant amb un punt a ~50 metres o més)
        for (let i = 10; i < points.length; i++) {
            const distDiff = points[i].distance - points[i-10].distance;
            if (distDiff <= 0) continue;

            const eleDiff = smoothedEle[i] - smoothedEle[i-10];
            let grade = (eleDiff / (distDiff * 1000)) * 100;
            
            // Limitació física realista per evitar xifres absurdes per errors de dades
            ...
            if (!inClimb) {
                ...
                if (grade < -1.0 || (smoothedEle[startIndex] > smoothedEle[i] && (smoothedEle[findPeakIndex(smoothedEle, startIndex, i)] - smoothedEle[i] > 30))) {
                    ...
                    // Filtres estrictes: mínim 1.5 km i 80m de desnivell real
                    if (climbDist >= 1.5 && climbEleGain >= 80 && avgGrade >= 3.0 && avgGrade <= 25) {
                        ...
                        ports.push({
                            ...
                            avgGrade: avgGrade,
                            maxGrade: Math.min(maxGradeInClimb, 30), // top màxim lògic
                            apm: apm,
                            category: category
                        });
                    }
    ...
    function getRealPortName(lat, lon, altitude) {
        const officialPorts = [
            { name: "Coll de la Creueta", lat: 42.298, lon: 1.988, alt: 1920, margin: 0.1 },
            { name: "Collada de Toses", lat: 42.320, lon: 2.010, alt: 1800, margin: 0.1 },
            { name: "Port del Cantó", lat: 42.370, lon: 1.340, alt: 1725, margin: 0.12 },
            { name: "Coll de la Gallina", lat: 42.440, lon: 1.430, alt: 1900, margin: 0.1 },
            { name: "Coll de Pal", lat: 42.310, lon: 1.890, alt: 2100, margin: 0.1 },
            { name: "Alt de la Comella", lat: 42.500, lon: 1.520, alt: 1045, margin: 0.08 },
            { name: "Coll d'Ordino", lat: 42.560, lon: 1.540, alt: 1980, margin: 0.1 },
            { name: "La Rabassa", lat: 42.430, lon: 1.470, alt: 2040, margin: 0.1 },
            { name: "Port del Compte", lat: 42.170, lon: 1.580, alt: 2000, margin: 0.12 }
        ];
        ...
        for (let port of officialPorts) {
            ...
            if (distLat <= port.margin && distLon <= port.margin && altDiff <= 250) {
                return port.name;
            }
        }
