...
helpBtn.addEventListener('click',()=>{const s=suggest(state.remainder,state.divisor);if(!s){helpText.textContent='No cal més.';return;}chunkInput.value=s;calcPreview.textContent=`${state.divisor} × ${s} = ${state.divisor*s}`;helpText.textContent=`Suggeriment: ${s}`});
