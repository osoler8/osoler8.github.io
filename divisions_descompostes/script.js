...
let state = {
  a: 0,
  b: 1,
  steps: [], // each step: {k, prod, after}
};

function fmt(n){ return Number(n).toString(); }

function renderAll() {
  els.dA.textContent = fmt(state.a);
  els.dB.textContent = fmt(state.b);
  els.work.innerHTML = '';
  let rem = state.a;
...
function suggestStep(rem, b) {
  ...
  if (q <= 0) return 0;
  // redueix a un número 'agradable' (múltiple de potència de 10)
  const pow = Math.pow(10, String(q).length - 1);
  const nice = Math.floor(q / pow) * pow;
